...
export default function ProductCard({ urun }: { urun: Urun }) {
  ...
  return (
    <div className="group relative rounded-xl border border-stone-200 bg-white p-3 transition hover:shadow-md">
      <button
        type="button"
      ...
      <button
        ...
        className="mt-3 w-full rounded-lg bg-stone-900 py-2 text-sm font-medium text-white transition hover:bg-stone-700 disabled:cursor-not-allowed disabled:bg-stone-300"
      >
        {urun.stok === 0 ? "Stokta Yok" : sepette ? "Sepetten Çıkar" : "Sepete Ekle"}
      </button>
    </div>
  );
}
